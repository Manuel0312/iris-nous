...
def test_home_shows_novita_button(tmp_path: Path) -> None:
    ...
    assert "Meno confusione mentre scorri" in page.text or "Chat ospite solo sul tuo dispositivo" in page.text
    assert "Chat ospite solo sul tuo dispositivo" in page.text
    assert "Scrivere sotto, come su WhatsApp" in page.text
